package org.firstinspires.ftc.teamcode.drive.SolarisOpModes;
...
public class RoadRunnerThreeRed extends LinearOpMode {
...
//            moveDistance(0.4, 700);
        ...
        // TODO: just for testing purposes we are assuming using the middle stone
        leftvaluedetect = 255;
        midvaluedetect = 0;
        rightvaluedetect = 255;

        // position of first skystone
        // left pos
        Vector2d FirstSkystoneLeftPos = new Vector2d(50,0);
        // mid pos
        Vector2d FirstSkystoneMidPos = new Vector2d(50, -20);

        // right pos
        Vector2d FirstSkystoneRightPos = new Vector2d(50,-30);

        // position the robot needs to assume it is in after it obtains the skystone
        Pose2d returnPositionFirstSkystone;
        // determine what skystone to strafe to
        if (leftvaluedetect == 0) {
            returnPositionFirstSkystone = new Pose2d(FirstSkystoneLeftPos.getX(),FirstSkystoneLeftPos.getY(),-90);
        }
        else if (midvaluedetect == 0) {
            returnPositionFirstSkystone = new Pose2d(FirstSkystoneMidPos.getX(),FirstSkystoneMidPos.getY(),-90);
        }
        else {
            returnPositionFirstSkystone = new Pose2d(FirstSkystoneRightPos.getX(),FirstSkystoneRightPos.getY(),-90);
        ...
        // strafe to first skystone

        drive.followTrajectorySync(
                drive.trajectoryBuilder()
                        .strafeTo(FirstSkystoneMidPos)
                        .build());
        ...
        // assume the robot is in the position infront of where it got the skystone i think
        drive.setPoseEstimate(returnPositionFirstSkystone);
        // drive to platform with stone
        // TODO: figure out why the heck i need to subtract this inorder to make it go strait lol
        drive.followTrajectorySync(
                drive.trajectoryBuilder()
                        .lineTo(new Vector2d(poseEstimate.getX()-60, -130))
                        .build());
        // strafe to platform
        drive.followTrajectorySync(
                drive.trajectoryBuilder()
        ...
        if (leftvaluedetect == 0) {
            drive.followTrajectorySync(
                    drive.trajectoryBuilder()
                            .reverse()
                            .lineTo(new Vector2d(poseEstimate.getX(), poseEstimate.getY()+39))
                            .build());
        }
        ...
        // skystone position right
            ...
            drive.followTrajectorySync(
                    drive.trajectoryBuilder()
                            .reverse()
                            .lineTo(new Vector2d(poseEstimate.getX(), poseEstimate.getY()+25))
                            .build());
        }
        ...
        drive.followTrajectorySync(
                drive.trajectoryBuilder()
                        .lineTo(new Vector2d(poseEstimate.getX()-60, poseEstimate.getY()-115))
                        .build());
